...
const Sidebar = () => {
  ...
  const getAllNavItems = (): NavItem[] => [
    ...
    { name: 'Bench', href: '/bench-list', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z', roles: ['ADMIN', 'HR_MANAGER'] },
  ...
  const navItems = getAllNavItems().filter(item => {
    // If no user or no roles specified, show all items (fallback for debugging)
  ...
  return (
    <div className="z-40 w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out lg:translate-x-0 translate-x-0">
        ...
        <ul className="space-y-1">
          {navItems.map((item) => (
            <li key={item.name}>
              <Link
                href={item.href}
                className={`${pathname === item.href ? 'bg-teal-50 text-teal-600 border-r-2 border-teal-600' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'} group flex items-center px-3 py-2.5 text-sm font-medium rounded-md transition duration-150 ease-in-out`}
              >
