document.addEventListener('DOMContentLoaded', () => {
    ...
    const saveInputs = () => {
        ...
        localStorage.setItem('performanceCriteria', JSON.stringify(currentCriteriaItems));
    };

    const loadInputs = () => {
        const savedCriteriaItems = JSON.parse(localStorage.getItem('performanceCriteria'));
        if (savedCriteriaItems && savedCriteriaItems.length > 0) {
            ...
            addCriterionItem('Quality of Work', 4, 30);
            addCriterionItem('Teamwork & Collaboration', 5, 25);
            addCriterionItem('Productivity', 4, 25);
            addCriterionItem('Initiative', 3, 20);
        }
    };
    ...
    const calculatePerformanceScore = () => {
        ...
        if (!isValid || totalWeight === 0) {
            performanceScoreSpan.textContent = 'Invalid Input';
            scoreInterpretationP.textContent = '';
            console.error('Analytics Event: Invalid input for performance score calculation');
            return;
        }

        const overallScore = totalWeightedScore / totalWeight;
        performanceScoreSpan.textContent = overallScore.toFixed(2);
        ...
        scoreInterpretationP.textContent = `Interpretation: ${interpretation}`;

        saveInputs();
        console.log(`Analytics Event: Performance score calculated - ${overallScore.toFixed(2)} (${interpretation})`);
    };

    calculateBtn.addEventListener('click', calculatePerformanceScore);

    loadInputs();
    calculatePerformanceScore(); // Initial calculation on page load
    ...
    subscribeBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('/data/tools/employee-performance-review-calculator-leads.json', {
                method: 'POST',
                headers: {
